...
def show_splash_screen(image_path: Path, duration_ms: int = 3000, max_size=(800, 800)):
    ...
    try:
        ...
        label.image = img # ← 参照保持（これがないとpyimage1エラーになる）
        label.pack()
...
def run_media_sort(input_folder: Path, output_folder: Path, splash: bool = False, splash_path: Path = None):
    """
    メディア整理処理を一括実行する関数（GUI/CLI 共通で使える入口）
    """

    file_list = scan_files(input_folder)
    if not file_list:
        logging.info("対象ファイルが見つかりませんでした。処理を終了します。")
        return

    logging.info(f"{len(file_list)} 件のファイルを検出しました。")
    for f in file_list:
        taken = get_taken_date(f)
        created = get_created_date(f)
        target_path = generate_target_path(f, taken, created, output_folder)
        move_file(f, target_path)

    logging.info("=== MediaSort 処理終了 ===")
